...
class TestFAQPage:
    @pytest.mark.parametrize("question_index, expected_answer", [(0,"Сутки — 400 рублей. Оплата курьеру — наличными или картой."),
    ...
    def test_questions_answers_open(self, driver, question_index, expected_answer):
        ...
        with allure.step(f"Opening question {question_index}"):
            faq.open_question(question_index)

        with allure.step(f"Getting answer for question {question_index}"):
            answer_faq = faq.get_answer(question_index)
